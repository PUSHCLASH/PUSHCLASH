# copy this entire file over your current app.py / flask_app.py
import os
import time
import json
import subprocess
...
def init_db():
    ...
    We use CREATE TABLE IF NOT EXISTS (safe) and then ALTER TABLE ... ADD COLUMN IF NOT EXISTS
    so upgrades from older DB schemas work when the app starts.
    """
    with app.app_context():
        ...
        try:
            ...
            # If ALTER fails for some reason, log but continue — table still usable without ghost_data
            app.logger.warning("Could not alter battles table to add ghost_data: %s", e)
        get_db().commit()
...
def ghost():
    ...
    cur = get_db().cursor()
    # ghost_data column is added in init_db if missing; selecting it is safe now
...
def target():
    ...
    # avg_db can be Decimal or None — cast to float safely
    try:
...
<style>
  /* All your existing CSS unchanged (omitted here for brevity in the message) */
  *{margin:0;padding:0;box-sizing:border-box;font-family:'Poppins',sans-serif}
  ...
  /* (the rest of CSS is identical to what you already had) */
</style>
</head>
<body>
...
<!-- The rest of the frontend HTML/JS is identical to what you already have,
     including the updated pose detection / rep-counting JS you previously asked for.
     For brevity I keep the same markup here — in your copy keep the same
     FRONTEND_HTML content you already tested that includes the JS rep-counting logic. -->

<!-- (Full page content omitted in this message to keep it compact; use the same FRONTEND_HTML you already deployed) -->
