...
<style>
  *{margin:0;padding:0;box-sizing:border-box;font-family:'Poppins',sans-serif}
  ...
  .countdown-warning{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:90%;background:rgba(0,0,0,0.9);border:2px solid #ff4500;border-radius:16px;padding:20px;text-align:center;z-index:10;display:none;box-shadow:0 0 30px #ff4500}
  .countdown-warning h3{color:#ff4500;font-size:1.2rem;margin-bottom:10px}
  .countdown-warning p{font-size:0.85rem;line-height:1.6;color:#ccc}
  .countdown-warning .emoji{font-size:1.5rem}

  /* CEO Badge and other styles unchanged */
  ...
  .ceo-modal-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.85);z-index:20000;display:none;align-items:center;justify-content:center}
...
<div class="active-users-pill"><span id="activeUserCount">0</span> online</div>
...
<div id="ceoModal" class="ceo-modal-overlay" onclick="this.classList.remove('active')"><div class="ceo-modal"><h2>KAUSHTUBH</h2><div class="title">CEO OF PUSH CLASH</div><div class="phone">📞 8950592855</div></div></div>
...
<div class="app-container" id="app">
  ...
  <div id="instructionScreen" class="screen"><h1>🚀 WELCOME!</h1><div class="instruction-box">...</div><button class="btn-primary" onclick="showScreen('dashboardScreen');loadStats()">⚡ ENTER ARENA ⚡</button></div>
  ...
  <div id="statsScreen" class="screen"><h1>📊 MY STATS</h1><button class="btn-secondary" onclick="showScreen('dashboardScreen')">← Back</button></div>
  <div id="challengeScreen" class="screen">
    <div class="countdown-warning" id="countdownWarning">
      <span class="emoji">⚠️🤖</span>
      <h3>AI IS WATCHING YOU, WARRIOR!</h3>
      <p>Your face is NOT shown on screen because we believe in EQUALITY — no judging by looks, only by power! The AI sees your motion, your angles, your struggle. Look directly into the camera and do proper push‑ups — the AI will count them automatically. No shortcuts, no mercy. Let's go! 💪🔥</p>
    </div>
    <div id="countdownDisplay" class="timer-big">3</div>
    <div id="challengeActiveUI" style="display:none"><div class="timer-big" id="timerDisplay">60</div><div class="counter-big" id="repCounter">0</div><div id="aiCameraUI"><video id="webcam" autoplay playsinline muted></video><canvas id="poseCanvas"></canvas><div class="angle-overlay" id="angleOverlay"></div><div class="rep-flash" id="repFlash">REP!</div><div class="debug-msg" id="debugMsg"></div></div></div>
  <div id="battleResultUI" style="display:none"><h2>⚔️ Battle Over!</h2><div id="finalScore">0</div><button class="btn-primary" onclick="shareScore()">📢 Share</button><button class="btn-secondary" onclick="goToDashboard()">Back</button></div>
...
<script>
let currentUser=null,repCount=0,timeLeft=60,challengeInterval,countdownInterval,aiDetector=null,aiStream=null,muteAI=false;
...
async function startChallenge(mode){
  ...
  speak("Warning! The AI is watching you. No face on screen — only your strength will be judged. Start pushing into the camera!");
  countdownInterval=setInterval(()=>{c--;if(c===0){document.getElementById('countdownDisplay').textContent='GO!';setTimeout(()=>{clearInterval(countdownInterval);document.getElementById('countdownDisplay').style.display='none';document.getElementById('countdownWarning').style.display='none';startActiveChallenge()},400)}else{document.getElementById('countdownDisplay').textContent=c;speak(c.toString())}},800)}
...
async function startAICamera(){
  ...
  try{
    ...
    debug.textContent='✅ Camera ready!';
  }catch(e){
    try{
      aiStream=await navigator.mediaDevices.getUserMedia({video:true});
      video.srcObject=aiStream;await video.play();
      debug.textContent='✅ Camera ready (basic)';
    }catch(e2){
      debug.textContent='❌ Camera denied. Check permissions.';return}
  }
  video.addEventListener('loadedmetadata',()=>{canvas.width=video.videoWidth||640;canvas.height=video.videoHeight||480;});
